...
//	memset( &cg, 0, sizeof( cg ) );
	...
	// Register tmLogin for autocomplete (available to all)
	trap->AddCommand("tmLogin");

	// Set userinfo key so the server knows we have the TarasciiMadness cgame (value = version)
	trap->Cvar_Register(NULL, "tm_cgame", "1.4", CVAR_USERINFO);

	// TarasciiMadness: disable HUD elements that don't apply
	if (cgs.gametype == GT_TEAM) {
		trap->Cvar_Set("cg_drawTeamOverlay", "0");
		trap->Cvar_Set("cg_drawRadar", "0");
		trap->Cvar_Set("cg_drawScores", "0");
	}
